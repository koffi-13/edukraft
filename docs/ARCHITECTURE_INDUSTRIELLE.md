...
> ## ⚠️ Note d'usage — lire avant d'implémenter
>
> Ce document est une **feuille de route technique** pour la montée en gamme
> d'EduKraft, pas un cahier des charges à implémenter tel quel.
>
> **Le correctif smart contract `verifyCertHash` O(1) (section 6) est IMPLÉMENTÉ**
> dans `contracts/contracts/EduKraftBadge.sol` — c'était le seul élément
> immédiatement actionnable et à valeur durable.
>
> **Les 3 piliers (EventBus gamification, DAG parcours, Quiz v2) sont des pistes
> pour v1.1/v2**, à prioriser selon la croissance réelle d'EduKraft.
>
> **La stack lourde proposée (PostgreSQL + Redis + TimescaleDB + BullMQ + Python
> FastAPI + Directus) est SURDIMENSIONNÉE** pour une PME togolaise avec SQLite +
> Express. Elle est présentée à titre indicatif pour visualiser l'architecture
> cible ultime — la stack actuelle (SQLite + Express + JS) suffit largement pour
> les 2 prochaines années. Ne pas migrer vers PostgreSQL/Python sans une
> justification claire (volume d'utilisateurs > 10 000 actifs, par exemple).
>
> **Priorisation recommandée** :
> 1. ✅ Fait — Correctif `verifyCertHash` O(1) + anti-doublon
> 2. 🟠 v1.1 — Source unique des achievements (déplacer ACHIEVEMENT_DEFS côté serveur)
> 3. 🟠 v1.1 — EventBus gamification côté client (mitt.js, 3 Ko — pas de BullMQ/Redis)
> 4. 🟡 v1.2 — Hooks spécialisés (scinder useDb en useLearner/useProgress/useGamification)
> 5. 🟡 v2 — Quiz versioning (question_version + content_hash, sans CMS Directus)
> 6. 🟢 v2+ — DAG parcours (uniquement quand > 15 modules interconnectés)
